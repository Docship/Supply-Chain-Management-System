...
exports.registerAssistant = async (req, res, next) => {
    ...
    userController
        .createUser(req, res, details, "ASSISTANT")
...
exports.deleteAssistant = async (req, res, next) => {
    ...
    userController
        .deleteUserAccount(req, res, userId, "ASSISTANT")
...
exports.postDeliveryComponents = async (req, res, next) => {
    ...
    if (
        trucks.result.length == 0 ||
        assistants.result.length == 0 ||
        routes.result.length == 0 ||
        drivers.result.length == 0
    ) {
        res.status(400).json({
            message: "Route not exists",
        });
...
// exports.truckOrderAssign1 = async (req, res, next) => {
//     const userId = req.user.id
//     const sql = storekeeperQuary.findStorekeeperByUserId()
//     let storekeeper = await dbConnection.findExecution(sql,[userId]);

//     if (storekeeper.status != 200) {
//         res.status(404).json({
//             message: "An error occurred",
//         });
//         return;
//     }
//     if (storekeeper.result.length == 0) {
//         res.status(404).json({
//             message: "empty storekeepers",
//         });
//         return;
//     }
//     const city = storekeeper.result[0]['City']

//     const ordersSql = orderQuary.findOrderforStartCityTruck();
//     let orders = await dbConnection.findExecution(ordersSql, [city]);
...
//     });
// };
// exports.truckOrderAssign2 = async (req, res, next) => {
//     const {
//         userId
//     } = req.body;

//     const storekeeperSql =storekeeperQuary.findStorekeeperByUserId();
//     let storekeeper = await dbConnection.findExecution(storekeeperSql, [userId]);
//     console.log(storekeeper)
//     const endCity = storekeeper.result[0]['City']

//     const ordersSql = orderQuary.findOrderforStartCityTruck();
//     let orders = await dbConnection.findExecution(ordersSql, [endCity]);

//     const assistantSql = assistantQuary.getAssistants()
//     let assistants = await dbConnection.findExecution(assistantSql, [endCity]);
    
//     const routeSql = routeQuary.getRoutesByStartCity()
//     let route = await dbConnection.findExecution(routeSql, [endCity]);

//     if (orders.status != 200 || assistants.status != 200 ||route.status != 200) {
//         res.status(404).json({
//             message: "An error occurred",
//         });
//         return;
//     }
//     if (orders.result.length == 0 ||assistants.result.length == 0 ||route.result.length == 0 ) {
//         res.status(404).json({
...
//         assistants:assistants.result,
//         route:route.result
//     });
// };


// exports.truckOrderAssign3 = async (req, res, next) => {
//     const {
//         truckNumber,
//         assistantId,
//         routeId,
...
//         const sql = truckOrderDeliveryQuary.insertTruckOrderDelivery();
//         const vars = [element, assignmentId];
//         const result = await dbConnection.insertExecution(sql, vars);
...
//             const deleteOrderAssignSql = truckOrderDeliveryQuary.deleteTruckOrderDeliveryByAssignId()
                
//             const deleteOrderDelSql = truckDeliveryAssignQuary.deleteOrderDeliveryByAssignId()
